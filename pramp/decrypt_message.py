...
# def decrypt_PRACTICE(word):
def decrypt(word):
    return

def decrypt_PASSED(word):
# def decrypt(word):
    retval = ""
...
def main():
    ######### TESTS ############
    test("crime", decrypt("dnotq"))
    test("encyclopedia", decrypt("flgxswdliefy"))
    test("qqqqq", decrypt("rajsb"))
    test("abcdefghijklmnopqrstuvwxyz", decrypt("bvqmjhgghjmqvbiqzjugthwmdv"))
    test("drugtrafficking", decrypt("eobamwpnlmhklrq"))
    test("", decrypt(""))
